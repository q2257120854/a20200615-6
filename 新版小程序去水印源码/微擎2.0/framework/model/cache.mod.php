...
function cache_build_account_modules($uniacid = 0, $uid = 0) {
	...
	foreach ($uni_modules_new as $uni_module_new_key => $uni_module_new_val) {
		...
		if (in_array($account_info['type'], array(ACCOUNT_TYPE_APP_NORMAL, ACCOUNT_TYPE_APP_AUTH)) && $uni_module_new_val[MODULE_SUPPORT_WXAPP_NAME] != MODULE_SUPPORT_WXAPP) {
			unset($uni_modules_new[$uni_module_new_key]);
		}
	}
	...
	if ($account_info->supportVersion) {
		...
		if (!empty($version_all)) {
			foreach($version_all as $version_key => $version_val) {
				...
				foreach ($version_val['modules'] as $module_name => $module_info) {
					$version_modules[$module_name] = $module_info;
				}
			}
		}
		$version_modules = array_keys($version_modules);
		...
		$uni_account_modules = table('uni_modules')->getModulesByUid(0, $uniacid);
	foreach ($uni_account_modules['modules'] as $uni_account_mdoule_key => $uni_account_module_val) {
		$uni_account_modules['modules'][$uni_account_module_val['module_name']] = $uni_account_module_val;
		unset($uni_account_modules['modules'][$uni_account_mdoule_key]);
	}
	$uni_account_modules = array_keys($uni_account_modules['modules']);
	...
	asort($uni_modules_add);
	asort($uni_modules_delete);
...
function cache_build_uninstalled_module() {
	...
	foreach ($module_path_list as $path) {
		...
		if (!empty($modulelist[$modulename])) {
			...
			if (!empty($module_cloud_upgrade)) {
				...
				foreach (module_support_type() as $support => $value) {
					...
					if ($module_cloud_upgrade[$support] == $value['support'] && $modulelist[$modulename][$support] == $value['not_support']) {
		...
		if (!empty($manifest['platform']['supports'])) {
			foreach (array('app', 'wxapp', 'webapp', 'android', 'ios', 'system_welcome', 'xzapp', 'aliapp') as $support) {
