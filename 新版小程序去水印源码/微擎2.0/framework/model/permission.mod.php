...
function permission_create_account($uid, $type = ACCOUNT_TYPE_OFFCIAL_NORMAL) {
	...
	$userinfo = table('users')->getById($uid);
	if (user_is_vice_founder($uid)) {
		$groupdata = table('users_founder_group')->getById($userinfo['groupid']);
	} else {
		$groupdata = table('users_group')->getById($userinfo['groupid']);
	}
...
function permission_user_account_num($uid = 0) {
	...
	$extra_create_group_info  = array_keys($extra_group_table->getCreateGroupsByUid($uid));
	$extra_limits_info = $extra_limit_table->getExtraLimitByUid($uid);
