...
if ($do == 'search_link_account') {
	...
	$allowed_type = array('account' => ACCOUNT_TYPE_OFFCIAL_NORMAL, 'wxapp' => ACCOUNT_TYPE_APP_NORMAL, 'webapp' => ACCOUNT_TYPE_WEBAPP_NORMAL, 'phoneapp' => ACCOUNT_TYPE_PHONEAPP_NORMAL, 'xzapp' => ACCOUNT_TYPE_XZAPP_NORMAL, 'aliapp' => ACCOUNT_TYPE_ALIAPP_NORMAL);
	if ($_W['account']['type_sign'] != 'wxapp') {
		unset($allowed_type[$_W['account']['type_sign']]);
	}
	if (!in_array($account_type, $allowed_type)) {
		iajax(0, array());
	}
		$link_sub_uniacids = table('uni_link_uniacid')->getAllSubUniacidsByModuleName($module_name);
		$account_list = uni_search_link_account($module_name, $account_type, $_W['uniacid']);
