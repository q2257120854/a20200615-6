...
if ($do == 'upgrade') {
	...
	if ($has_new_support) {
		foreach ($support as $val) {
			$module[$val] = $module_upgrade[$val];
		}
		$module_upgrade['account_support'] = $module['account_support'];
  		$module_upgrade['wxapp_support'] = $module['wxapp_support'];
  		$module_upgrade['webapp_support'] = $module['webapp_support'];
  		$module_upgrade['phoneapp_support'] = $module['phoneapp_support'];
  		$module_upgrade['xzapp_support'] = $module['xzapp_support'];
  		$module_upgrade['aliapp_support'] = $module['aliapp_support'];
  		$module_upgrade['welcome_support'] = $module['welcome_support'];
	}
...
if ($do =='install') {
	...
	$all_support = array('account_support', 'wxapp_support', 'webapp_support', 'phoneapp_support', 'xzapp_support', 'aliapp_support', 'welcome_support');
	...
	foreach ($all_support as $support) {
		if (!in_array($support, $module_support_name_arr)) {
			$module[$support] = MODULE_NONSUPPORT_ACCOUNT;
		}
	}
