...
$dos = array('platform', 'system', 'ext', 'account_ext', 'get_fans_kpi', 'get_last_modules', 'get_system_upgrade', 'get_upgrade_modules', 'get_module_statistics', 'get_ads', 'get_not_installed_modules', 'system_home', 'set_top', 'set_default', 'add_welcome', 'ignore_update_module', 'get_workerorder', 'add_welcome_shortcut', 'remove_welcome_shortcut');
...
if ($do == 'platform') {
	...
	cache_build_account_modules($_W['account']['uniacid']);
...
if ($do == 'ext') {
	...
	if ($_GPC['system_welcome'] && $_W['isfounder']) {
		...
		if ($modulename != 'store' && empty($_W['current_module']['main_module'])) {
...
if ($do == 'system_home') {
	...
	$own_account_modules_show = $uni_modules_table->searchWithPage(1, 5)->getModulesByUid($_W['uid']);
	$uni_modules_table->searchGroupbyModuleName();
	$own_account_modules_all = $uni_modules_table->getModulesByUid($_W['uid']);

	if (!empty($own_account_modules_all)) {
		foreach($own_account_modules_all['modules'] as $m_a_key => &$m_a_val) {
			$m_a_val['logo'] = tomedia($m_a_val['logo']);
			$m_a_val['checked'] = 0;
			$own_account_modules_all['modules'][$m_a_key] = $m_a_val;

			if ($m_a_val['role'] == 'clerk') {
				$user_module_permission_info = table('users_permission')->getUserPermissionByType($_W['uid'], $m_a_val['uniacid'], $m_a_val['module_name']);
				if (!$user_module_permission_info) {
					unset($own_account_modules_all['modules'][$m_a_key]);
				}
			}
	...
	if (!empty($user_welcome_modules)) {
		...
		if (empty($last_modules) && !empty($own_account_modules_show)) {
		$is_empty_modules = 1;
		foreach ($own_account_modules_show['modules'] as $m_key => $m_val) {
