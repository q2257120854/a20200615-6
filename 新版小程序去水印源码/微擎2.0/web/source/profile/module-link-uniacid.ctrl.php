...
if ($do == 'search_link_account') {
	...
	if (!empty($uniacid)) {
		$uni_account = uni_fetch($uniacid);
		if (empty($uni_account)) {
			iajax(1, '账号不存在或已删除');
		}
	}
	$allowed_type = array(
		'account' => ACCOUNT_TYPE_OFFCIAL_NORMAL,
		'wxapp' => ACCOUNT_TYPE_APP_NORMAL,
	...
	if (!empty($uni_account) && $uni_account->typeSign != WXAPP_TYPE_SIGN) {
		unset($allowed_type[$uni_account->typeSign]); 	}
	if (!in_array($account_type, $allowed_type)) {
		iajax(1, '参数有误');
	}
		$link_sub_uniacids = table('uni_link_uniacid')->getAllSubUniacidsByModuleName($module_name);
		$account_list = uni_search_link_account($module_name, $account_type, $uniacid);
