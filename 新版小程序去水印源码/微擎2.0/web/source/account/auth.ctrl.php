...
if ($do == 'forward') {
	...
	$auth_refresh_token_exist = pdo_get('account_wechats', array('auth_refresh_token' => $auth_refresh_token));
	if ($auth_refresh_token_exist) {
		itoast('授权刷新token已存在，请重新授权', url('account/manage', array('account_type' => ACCOUNT_TYPE_OFFCIAL_NORMAL)), 'error');
	}
