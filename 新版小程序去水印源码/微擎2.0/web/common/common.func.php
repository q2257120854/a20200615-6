...
function buildframes($framename = ''){
	...
	if ((!empty($modulename) || !empty($eid)) && !in_array($modulename, module_system())) {
			...
			if (!empty($plugin_list)) {
				...
				foreach ($plugin_list as $plugin_module) {
					...
					$frames['account']['section']['platform_module_plugin']['menu']['platform_' . $plugin_module_info['name']] = array(
						...
						'url' => url('account/display/switch', array('module_name' => $plugin_module_info['name'], 'uniacid' => $_W['uniacid'])),
						'is_display' => 1,
					);
					$i++;
				}
		...
		load()->model('phoneapp');
		$version_id = intval($_GPC['version_id']);
		if ($_W['account']->typeSign == 'phoneapp') {
			$wxapp_version = phoneapp_version($version_id);
		} else {
		...
		'<div class="we7-body-alert">'+
			'<div class="container">'+
				'<div class="alert alert-info">'+
					...
					'<a href="{$url}" target="_blank">请尽快完善您在云服务平台的站点注册信息。</a>'+
				'</div>'+
			'</div>'+
		'</div>';
	$('body').prepend(html);
