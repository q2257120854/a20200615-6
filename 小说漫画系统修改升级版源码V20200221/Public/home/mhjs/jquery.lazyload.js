    ...
    $.ajax({
        ...
        success: function(data) {
            try {
                ...
                window.console && window.console && console.log("页面动态加载js异常", e);
            }
        },
        error: function(xhr, type) {
            try {
                ...
                window.console && window.console && console.log("页面动态加载不成功", e);
            }
        },
    })
    return false;
...
function trigger_lazy_ajax() {
    if (window.$ && $.fn && $.fn.lazyload) {
        $(".img-ajax-lazy").lazyload({
            ...
            effect: 'fadeIn',
            load: function(x, y) {
                $(this).removeClass('img-ajax-lazy');
            }
        });
    } else {
...
function get_other_books_info(bid) {
    ...
    AjaxJson(url, data, function(data) {
        if (data.status * 1 == 1) {
            ...
            $("img.cal-placeholder").each(function(i, x) {
                x.width > 0 && x.height && $(x).css({
                    width: x.width,
                    height: x.height
                });
            });
...
function publist_comment() {
    ...
    AjaxJson(publist_url, publist_data, function(res) {
        ...
        if (res.status * 1 == 1) {
            ...
            if (btype == 1) {
                setTimeout(function() {
                    window.location.href = '/comment/list_comment/?novel_id=' + bid;
                }, 500);
            } else {
                setTimeout(function() {
                    window.location.href = '/comment/list_comment/?book_id=' + bid;
                }, 500);
            }
        } else {
            bh_msg_tips(res.info);
    ...
    (function($, window, document, undefined) {
        ...
        $.fn.lazyload = function(options) {
            ...
            var settings = {
                ...
                appear: null,
                load: null,
                placeholder: "//res.efucms.com/wap_v2/images/placeholder.png"
            };
