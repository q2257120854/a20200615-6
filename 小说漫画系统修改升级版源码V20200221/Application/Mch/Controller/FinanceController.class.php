...
class FinanceController extends AdminController {
	...
	public function charge(){
		...
		$this->assign('tAmoney',M('charge')->where($where)->sum('money'));
		//今日已支付充值
		$where['status'] = 2;
		$this->assign('tymoney',M('charge')->where($where)->sum('money'));
		//今日未支付充值
		$where['status'] = 1;
		$this->assign('twmoney',M('charge')->where($where)->sum('money'));
		...
		unset($where['status']);
		$this->assign('zAmoney',M('charge')->where($where)->sum('money'));
		//昨日已支付充值
		$where['status'] = 2;
		$this->assign('zymoney',M('charge')->where($where)->sum('money'));
		
		//昨日未支付充值
		$where['status'] =1;
		$this->assign('zwmoney',M('charge')->where($where)->sum('money'));
		...
		unset($where['status']);
		$this->assign('yAmoney',M('charge')->where($where)->sum('money'));
		//本月已支付充值
		$where['status'] = 2;
		$this->assign('yymoney',M('charge')->where($where)->sum('money'));
		//本月未支付充值
		$where['status'] = 1;
		$this->assign('ywmoney',M('charge')->where($where)->sum('money'));
		...
		unset($where['status']);
		$this->assign('lAmoney',M('charge')->where($where)->sum('money'));
		
		//累计已支付充值
		$where['status'] = 2;
		$this->assign('lymoney',M('charge')->where($where)->sum('money'));
		//累计未支付充值
		$where['status'] = 1;
		$this->assign('lwmoney',M('charge')->where($where)->sum('money'));
