...
<div class="navbar flt">
    <nav class="tab-box">
        ...
        <div class="item">
		 ...
		 <div class="item">
            <a href="<?php echo U('Yook/index');?>" class="active">听书</a>
        </div>
		<div class="item">
...
<div id="banner-container">
    ...
    <div class="indexnav portal-slick mt-navbar">
        <?php if(is_array($_ybanner['config'])): $i = 0; $__LIST__ = $_ybanner['config'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="item">
...
<nav class="portal-nav">
	<?php if(is_array($_yook)): foreach($_yook as $k=>$v): if($v['show'] == 1 and $v['isshow']): ?><div class="item">
...
<?php if(is_array($bookcate)): foreach($bookcate as $k=>$v): ?><div class="bm-box mt-10">
		...
		<div class="books-row">
			<?php if(is_array($v['list'])): $i = 0; $__LIST__ = $v['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="item">
				<a href="<?php echo U('bookinfo');?>&yid=<?php echo ($vo["id"]); ?>">
...
<?php if($mf): ?><div class="bm-box mt-10">
	...
	<div class="books-row">
		<?php if(is_array($mf)): $i = 0; $__LIST__ = $mf;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div style="width:100%;padding-bottom: 20px;border-bottom: 1px solid #ddd;position:relative;">
				...
				<div onclick="location.href=&#39;<?php echo U('bookinfo');?>&yid=<?php echo ($vo["id"]); ?>&#39;" style="position: absolute;top: 10px;right: 10px;padding: 8px;border: 1px solid #FF9800;border-radius: 5px;color: #FF9800;">立即阅读</div>
			</div><?php endforeach; endif; else: echo "" ;endif; ?>
	</div>
</div>
<?php else: ?>
