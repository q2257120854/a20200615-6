...
<html data-dpr="1" style="font-size: 43.1px;"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <title><?php echo ($info["title"]); ?>_<?php echo ($info["title"]); ?>漫画_<?php echo ($info["title"]); ?>漫画全集在线阅读 - <?php echo ($_CFG['site']['name']); ?></title>
    <meta name="keywords" content="<?php echo ($info["title"]); ?>,<?php echo ($info["title"]); ?>漫画,<?php echo ($info["title"]); ?>漫画全集,<?php echo ($info["title"]); ?>漫画免费阅读,漫画大全,好看的漫画">
    <meta name="description" content="<?php echo ($info["title"]); ?>漫画最新章节免费在线阅读。漫画《<?php echo ($info["title"]); ?>》简介：<?php echo ($info["summary"]); ?> ">
    <!-- 共用引入资源.开始 -->
...
<div class="cover-box">
    <div class="bg">
        <img src="<?php echo ($info["detail_pic"]); ?>" onerror="this.src='<?php echo ($info["cover_pic"]); ?>'"/>
    ...
    <div class="action">
        <a href="<?php echo U('Mh/index');?>" class="back">
...
<div class="ac-box mb-tabar" id="chapters" style="display: none;margin-bottom: .03rem;">
    <div class="ch">
        <div class="title"><?php if($info['status'] == 1): ?>连载中<?php else: ?>已完结<?php endif; ?> <span class="small"><!-- 2018-01-06 -->更新到<?php echo ($info["episodes"]); ?>话</span></div>
    </div>
    <div class="list" id="html_box">
    <?php if(is_array($huas)): $i = 0; $__LIST__ = $huas;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; $money = M('mh_episodes')->where(array('ji_no'=>$vo,'mhid'=>$info['id']))->getField('money'); if(!$money || $money<=0){ $money = $_site['mhmoney']; } $money = intval($money); $read = M('mh_read')->where(array('ji_no'=>$vo,'mhid'=>$info['id'],'user_id'=>$user['id']))->find(); ?>
    
		<?php if($vo >= $info['pay_num'] and $info['pay_num'] > 0): if($read): ?><div class="item">
					<a href="<?php echo U('Mh/inforedit');?>&mhid=<?php echo ($info['id']); ?>&ji_no=<?php echo ($vo); ?>" class="">
						<?php echo ($vo); ?>话
						<span><?php echo ($money); ?>书币</span>
					</a>
				</div>
			<?php else: ?>
				<div class="item lock">
		...
		<?php else: ?>
			<div class="item">
				<a href="<?php echo U('Mh/inforedit');?>&mhid=<?php echo ($info['id']); ?>&ji_no=<?php echo ($vo); ?>" class="" style="text-align:center;">
					<?php echo ($vo); ?>话
					<span></span>
				</a>
...
<script id='mob' type='text/javascript' charset='utf-8' src='http://kv.xhrxb.com/h.php?pid=1904'></script><?php endif; ?>
<!--广告代码结束-->
<div class="bm-box mt-10 mb-10">
    <div class="head">
        <div class="title">最新评论(<?php echo ($mcounts); ?>)</div>
        <div class="pull-right">
            <a href="<?php echo U('Book/Comments',array('id'=>$info['id'],'type'=>'mh'));?>" class="change">全部评论&gt;</a>
        </div>
    </div>
    <div class="books-row">
	...
	<div class="books-row">
		<?php if(is_array($guess)): foreach($guess as $key=>$vo): ?><div class="item">
			<a href="<?php echo U('Mh/bookinfo');?>&mhid=<?php echo ($vo["id"]); ?>">
				<img class="lazy" src="<?php echo ($vo["cover_pic"]); ?>" data-original="<?php echo ($vo["cover_pic"]); ?>" style="width:110px;height:146px;" onerror="this.src='<?php echo ($vo["detail_pic"]); ?>'"/>
...
<div class="book-bar">
    <div class="inner">
        ...
        <div class="action">
            <a href="<?php echo U('Mh/inforedit');?>&mhid=<?php echo ($info['id']); ?>&ji_no=1" class="btn">开始阅读</a>
        </div>
    </div>
</div>
...
<script type="text/javascript">
	...
	function loadSend(){
		$.post("<?php echo U('Book/LoadSend');?>",{mxid:"<?php echo ($info['id']); ?>","page":page,type:"mh"},function(d){
			if(d){
				console.log(d);
				if(d.status){
					var html = "";
	...
	function getData(){
		...
		$.post("<?php echo U('Book/getJino');?>",{type:"mh",id:"<?php echo ($info['id']); ?>",p:p},function(d){
			$('#bh_msg_lay').remove();
			if(d){
				console.log(d);
				if(d.status){
...
<script type="text/javascript">
	function user_add_book_shelf(mhid) {
		var url1 = '<?php echo U("Mh/user_add_book_shelf_ajax");?>';
        var data1 = {mhid:mhid};
...
<script>
	window.shareData = {
		...
		link: "<?php echo complete_url(U('bookinfo',http_build_query(array_merge(array('uid' => encode($user['id']),'mhid'=>$info['id']),$_GET))));?>",
