...
class VideoController extends HomeController {
    ...
    public function index(){
    	foreach ($this->_video as $k=>$v){
			if($v['show'] == 2 && $v['isshow']){
				$bookcate[$k]['name'] = $v['name'];
				$bookcate[$k]['list'] = M('video')->where(array('bookcate'=>array('like','%'.$k.'%')))->order('sort desc')->limit(6)->select();
			}
		}
		$this->assign('bookcate',$bookcate);
		$this->assign('mf',M('video')->where(array('free_type'=>1))->order('sort desc')->select());
    ...
    public function bookinfo(){
    	$vid = I('vid', 'intval', 0);
    	if(empty($vid)) {
    		$this->error('非法访问动漫数据！', U('Book/index'));
    	}
    	
    	$info = M('video')->where("id={$vid}")->find();
    	if(empty($info)) {
    		$this->error('动漫数据缺失！', U('index'));
    	}
    	...
    	if(session('user_id') > 0 || $this->user['id']) {
	    	$old = M('mh_collect')->where(array("mhid"=>$vid , "user_id"=>$this->user['id'],"type"=>"ys"))->find();
    ...
    public function book_cate(){
    	...
    	$list = M('video')->where($cond)->order('sort desc')->select();
    	$asdata = array(
    			'list'		=> $list,
    			'selfurl'	=> __SELF__,
    			'cateid'	=> $cateid,
    ...
    public function book_free(){
    	$list = M('video')->where("free_type=1")->order('sort desc')->limit(50)->select();
    ...
    public function book_hot(){
    	$list = M('video')->order('sort desc')->limit(50)->select();
    	if(!empty($list) && is_array($list)) {
    		foreach ($list as $k => &$v) {
    			$arr_catename = '';
    			$cateids = $v['cateids'];
    ...
    public function book_last(){
    	...
    	$list = M('video')->where("")->order('sort desc')->limit(50)->select();
    	if(!empty($list) && is_array($list)) {
    		foreach ($list as $k => &$v) {
    			$arr_catename = '';
    			$cateids = $v['cateids'];
    ...
    public function booklist(){
    	...
    	$list = M('video')->where($where)->order('is_new desc,id desc')->limit(50)->select();
	...
	public function getJino(){
		if(IS_POST){
			...
			$vid = I('post.vid');
			$start = ($page - 1)*20;
			$list = M('video_episodes')->where(array('vid'=>$vid))->order('ji_no asc')->limit($start,20)->select();
		    ...
		    if($list){
			   ...
			   foreach($list as $v){
				    ...
				    if(!$money || $money<0 || $money == 0){
					 ...
					 $read = M('book_ys_read')->where(array('user_id'=>$this->user['id'],'vid'=>$vid,'ji_no'=>$v['ji_no']))->find();
					if($read){
						$isread = 1;
					}else{
						$isread = 2;
   ...
   public function paynumPlay(){
	   if(IS_POST){
		   ...
		   $read = M('book_ys_read')->where(array('user_id'=>$this->user['id'],'vid'=>$vid,'ji_no'=>$jino))->find();
		   
		    if(!$read && $this->user['vip']!=1){
			    $info = M('video_episodes')->where(array('vid'=>$vid,'ji_no'=>$jino))->find();
    ...
    public function search(){
    	...
    	$list = M('video')->where($cond)->order('sort desc')->limit(50)->select();
    	$asdata = array(
    			'key'	=> $key,
    			'list'	=> $list,
    	);
    ...
    public function user_add_book_shelf_ajax() {
    	...
    	if($user_id > 0) {
    		$binfo = M('video')->where("id={$vid}")->find();
    		$old = M('mh_collect')->where(array("mhid"=>$vid,"user_id"=>$this->user['id'],'type'=>"ys"))->find();
    		if(empty($old)) {
    			$ins = array(
    					'mhid'		=> $vid,
    			...
    			M('book')->where("id={$vid}")->setInc('collect', 1);
    		} else {
    			M('mh_collect')->where(array("mhid"=>$vid,"user_id"=>$this->user['id'],'type'=>"ys"))->delete();
    			M('book')->where("id={$vid}")->setDec('collect', 1);
    			$tag = 2;//取消收藏成功
    		}
    	} else {
    		$status = 2;
