...
class MhController extends HomeController {
    ...
    //  return;
		...
		$this->assign('rq',M('mh_list')->order('reader desc')->limit(6)->select());
        $this->assign('jx',M('mh_list')->where(array('is_recomm'=>1))->order('id desc')->limit(6)->select());
    ...
    public function book_hot(){
		...
		if($order){
			if($order == "reader"){
				$order = "reader desc";//点击榜
			}
			if($order == "likes"){
				$order = "likes desc";//点赞榜
			}
			if($order == "send"){
				$order = "send desc";//打赏榜
			}
			...
			//if($order == "cate1"){
			//	$where['mhcate'] = array('like','%6%');
			//	$order = "sort desc";
			//}
			//if($order == "cate2"){
			//	$where['mhcate'] = array('like','%7%');
			//	$order = "sort desc";
			//}
		}else{
			$order = "sort desc";
    ...
    public function book_last(){
    	...
    	$list = M('mh_list')->where("")->order('id desc')->limit(50)->select();
    	if(!empty($list) && is_array($list)) {
    		foreach ($list as $k => &$v) {
    			$arr_catename = '';
    			$cateids = $v['cateids'];
    ...
    public function book_recomm(){
    	$list = M('mh_list')->where(array('is_recomm'=>1))->order('sort desc')->limit(50)->select();
    	if(!empty($list) && is_array($list)) {
    		foreach ($list as $k => &$v) {
    			$arr_catename = '';
    			$cateids = $v['cateids'];
	...
	public function payAli(){
        ...
        $c = new \AopClient();
		 ...
		 $notify_url="http://".$_SERVER['HTTP_HOST'].__ROOT__."/third/chinaxingNotify.php";
        $c->gatewayUrl = "https://openapi.alipay.com/gateway.do";
        $c->appId = $config['app_id'];
        $c->rsaPrivateKey = $merchant_private_key;
        $c->format = "json";
		//$c->method = "alipay.trade.wap.pay";
        $c->charset= "UTF-8";
        $c->signType= "RSA2";
