...
<head>
    ...
    <title>小鱼授权系统-安装</title>
    <link href="https://bootswatch.com/3/paper/bootstrap.min.css" rel="stylesheet">
</head>
<body background="./assets/img/xiaoyu.png">
<div class="container"><br>
    <div class="row">
        <div class="col-xs-12 col-sm-10 col-md-8 col-lg-6 center-block text-center" style="float: none;">
            <div class="panel-heading"style='background:linear-gradient(to right,#66FFFF,#14b7ff,#CC00FF,#FFCC33);'><font color='#fff'><center>盗版入库授权都没问题，小鱼授权系统更懂你...</center></div></font><br>
        </div>
...
<div class="panel panel-black">
	...
	<div class="panel-heading"style='background:linear-gradient(to right,#66FFFF,#14b7ff,#CC00FF,#FFCC33);'>
		<h3 class="panel-title" align="center">小鱼授权系统-安装</h3>
	</div>
	<div class="panel-body">
	<p><iframe src="../readme.php?r=<?php echo time()?>" style="width:100%;height:465px;"></iframe></p>
<?php if ($installed) { ?>
		<div class="alert alert-warning">您已经安装过小鱼授权系统，如需重新安装请删除<font color=red> “install/xiaoyulock” </font>文件后再安装！</div>
		<?php }else{?>
		<p align="center"><a class="btn btn-primary btn-block"style='background:linear-gradient(to right,#66FFFF,#14b7ff,#CC00FF,#FFCC33);'" href="?do=1">开始安装</a></p>
		<?php }?>
	</div>
</div>
...
echo <<<HTML
		...
		<input type="text" class="form-control" name="db_qqh">
...
if(defined("SAE_ACCESSKEY") || $_GET['jump']==1){
	if(defined("SAE_ACCESSKEY"))include_once '../system/sae.php';
	else include_once '../system/config.php';
	if(!$user||!$pwd||!$dbname) {
		...
		if(!$con=DB::connect($host,$user,$pwd,$dbname,$port)){
			...
			if(DB::query("select * from card_config where 1")==FALSE)
	...
	$db_qqh=isset($_POST['db_qqh'])?$_POST['db_qqh']:NULL;
	
	if($db_host==null || $db_port==null || $db_user==null || $db_pwd==null || $db_name==null || $db_qqh==null){
...
\$pwd = '{$db_pwd}'; //数据库密码
\$dbname = '{$db_name}'; //数据库名
\$zzqqname = '{$db_qqh}'; //获取QQ号

/*目录配置*/
define('CACHE_DIR','cache'); //下载缓存目录
define('PACKAGE_DIR','download'); //程序安装包目录
?>";
		if(!$con=DB::connect($db_host,$db_user,$db_pwd,$db_name,$db_port)){
			...
			elseif(DB::connect_errno()==1049)
				...
				echo '<div class="alert alert-warning">连接数据库失败，['.DB::connect_errno().']'.DB::connect_error().'</div>';
		}elseif(file_put_contents('../system/config.php',$config)){
			echo '<div class="alert alert-success">数据库配置文件保存成功！</div>';
...
if(!$user||!$pwd||!$dbname) {
	...
	require './db.class.php';
	$sql=file_get_contents("xiaoyu.sql");
		$sql=explode(';',$sql);
	$cn = DB::connect($host,$user,$pwd,$dbname,$port);
...
<div class="progress progress-striped">
  <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 100%">
	...
	<div class="panel-body">
<?php
	@file_put_contents("xiaoyulock",'安装锁');
	echo '<div class="info"><font color="green">安装完成！管理账号和密码是:admin/admin</font><br/><br/><a href="../">>>网站首页</a>｜<a href="../xiaoyu/">>>后台管理</a><hr/>更多设置选项请登录后台管理进行修改。<br/><br/><font color="#FF0033">如果你的空间不支持本地文件读写，请自行在install/ 目录建立 xiaoyulock 文件！</font></div>';
?>
	</div>
</div>
...
<?php
	@file_put_contents("xiaoyulock",'安装锁');
	echo '<div class="info"><font color="green">安装完成！管理账号和密码是:xiaoyu/123456</font><br/><br/><a href="../">>>网站首页</a>｜<a href="../xiaoyu/">>>后台管理</a><hr/>更多设置选项请登录后台管理进行修改。<br/><br/><font color="#FF0033">如果你的空间不支持本地文件读写，请自行在install/ 目录建立 xiaoyulock 文件！</font></div>';
?>
	</div>
</div>
