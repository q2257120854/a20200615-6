...
<div class="panel-heading font-bold">邀请码生成</div>
...
if ($_POST['do'] == 'do') {
	...
	if ($num != '') {
		for ($i=1;$i<=$num;$i++) {
			$key = getkmkey();
			$DB->query("INSERT INTO `xn_kms` (`km`, `zt`, `addid`) VALUES ('$key', '1','$uid')");
			
		$DB->query("insert into `auth_log` (`uid`,`type`,`date`,`city`,`data`) values ('".$udata['user']."','生成邀请码','".$date."','".$city."','卡密".$key."|数量".$num."')");
...
$pagesize=30;
$rs=$DB->query("SELECT * FROM xn_kms WHERE addid = ".$udata['uid']." order by id desc limit $pageu,$pagesize");
