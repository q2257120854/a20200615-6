...
class CACHE {
	...
	public function read() {
		...
		global $DB;
		$row=$DB->get_row("SELECT v FROM auth_pay WHERE k='cache' limit 1");
		return $row['v'];
	}
	public function save($value) {
		...
		return $DB->query("update auth_pay set v='$value' where k='cache'");
	}
	...
	public function update() {
		global $DB;
		$cache = array();
		$query = $DB->query('SELECT * FROM auth_pay where 1');
	...
	public function clear() {
		global $DB;
		return $DB->query("update auth_pay set v='' where k='cache'");
	}
}
