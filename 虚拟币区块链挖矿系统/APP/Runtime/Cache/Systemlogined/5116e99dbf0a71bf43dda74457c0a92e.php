				...
				<div id="page-content" class="clearfix">
					<div class="page-header position-relative">
						<div style="text-align:left;width:100%;">
						    <a type="button" href="<?php echo U(GROUP_NAME.'/Info/addAnnounceType');?>" class="btn btn-info btn-small no-border"> <i class="icon-plus icon-white"></i> 添加公告类别</a>
						</div>
					</div><!--/.page-header-->
									...
									<tbody>
										<?php if(is_array($type)): foreach($type as $key=>$v): ?><tr>
												...
												<td>
													...
													<button uid="<?php echo ($v["id"]); ?>" class="btn btn-mini btn-danger">
		...
		<script type="text/javascript">
			$(function() {
				...
				$('.btn-info').click(function(){
					...
					var url = '<?php echo U(GROUP_NAME .'/Info/editAnnounceType');?>'+'/id/'+ uid;
					window.location = url;
				});
				...
				$('.btn-danger').click(function(){
					...
					bootbox.confirm("确认删除吗?", function(result) {
						if(result) {
							var url = '<?php echo U(GROUP_NAME .'/Info/deleteAnnounceType');?>'+'/id/'+ uid;
							window.location = url;
						}
					});
