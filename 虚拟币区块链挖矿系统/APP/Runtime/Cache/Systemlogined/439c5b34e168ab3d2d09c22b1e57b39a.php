			...
			<div id="main-content" class="clearfix">
				...
				<div id="page-content" class="clearfix">
					...
					<div class="row-fluid">
						...
						<!--<div class="row-fluid">
                        <form method="post" action="<?php echo U('Member/setuser');?>">
                          <div style=" overflow:hidden;">
                        	<select name="set_type" style="float:left; height:33px; margin-right:5px;">
                            	<option value="1">封停</option>
                                <option value="2">解封</option>
                            </select>	
                           <input type="submit" value="确定" class="btn btn-small no-border" style="float:left;"> 
                        </div>-->
							...
							<table id="table_report" class="table table-striped table-bordered table-hover">
								<thead>
									<tr>
										...
										<th><a href="<?php echo U('systemlogined/Member/check',array('_order'=>'gamecount','_sort'=>0));?>">团队人数↑</a></th>
										<th>注册时间</th>
										<th>审核时间</th>
										<th><a href="<?php echo U('systemlogined/Member/check',array('_order'=>'mygonglv','_sort'=>0));?>">个人算力↑</a></th>
										<th><a href="<?php echo U('systemlogined/Member/check',array('_order'=>'teamgonglv','_sort'=>0));?>">公会算力↑</a></th>
										<th><a href="<?php echo U('systemlogined/Member/check',array('_order'=>'jinbi','_sort'=>0));?>">钱包结余↑</a></th>
										<th><a href="<?php echo U('systemlogined/Member/check',array('_order'=>'qjinbi','_sort'=>0));?>">冻结钱包↑</a></th>
										<th>状态</th>
                                        <th>在线状态</th>
                                        <th><a href="<?php echo U('systemlogined/Member/check',array('_order'=>'shopstatus','_sort'=>0));?>">商户状态↑</th>
                                        <th>操作</th>
									</tr>
								</thead>
								<tbody>
									<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
											...
											<td><?php echo ($v["gamecount"]); ?></td>
											<td><?php echo (date('Y-m-d H:i',$v["regdate"])); ?></td>
											<td><?php echo (date('Y-m-d H:i',$v["checkdate"])); ?></td>
											<td><?php echo ($v["mygonglv"]); ?></td>
											<td><?php echo ($v["teamgonglv"]); ?></td>
											<td><?php echo ($v["jinbi"]); ?></td>
											<td><?php echo ($v["qjinbi"]); ?></td>	
											<td>
												<?php if($v["lock"]): ?>已封停
												<?php else: ?>
													正常<?php endif; ?>
											...
											<td>
												<?php if($v["shopstatus"] == 0): ?>未开通<?php endif; ?>
												<?php if($v["shopstatus"] == 1): ?><font color='#0000FF'>已开通</font><?php endif; ?>
												<?php if($v["shopstatus"] == 2): ?><font color='red'>已封停</font><?php endif; ?>
											</td>
											...
											<td><?php if($v["lock"]): ?><a onclick="if(confirm('确认解封此账户吗?')==false)return false;" href="<?php echo U(GROUP_NAME .'/Member/editFeng',array('id'=>$v['id'],'lock'=>0));?>">解封</a>
												...
												| 
												<?php if(($v["shopstatus"] == 0) or ($v["shopstatus"] == 2)): ?><a  onclick="if(confirm('确认给此账户开通商户权限吗?')==false)return false;" href="<?php echo U(GROUP_NAME .'/Member/editshop',array('id'=>$v['id'],'shopstatus'=>1));?>">开通商户</a><?php endif; ?>
												<?php if($v["shopstatus"] == 1): ?><a onclick="if(confirm('确认查封此账户商户权限吗?')==false)return false;" href="<?php echo U(GROUP_NAME .'/Member/editshop',array('id'=>$v['id'],'shopstatus'=>2));?>">查封商户</a><?php endif; ?>
												| 
