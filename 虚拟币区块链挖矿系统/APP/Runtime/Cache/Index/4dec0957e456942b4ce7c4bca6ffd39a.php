...
<!-- <div class="box-body chart-responsive" style="background-image: url('__PUBLIC__/user2/images/kx.png');"> -->
	...
	<div style="width: 90%;margin-left: 5%;margin-top: 20px;overflow:hidden;border-radius:5px">
		<p id="qiugou_list"onclick='showhidediv("qiugou")'style="float: left;width: 50%;text-align: center;background-color:#1fc811;height: 30px;line-height: 30px;color:#fff;border-bottom-left-radius: 5px;border-top-left-radius: 5px">买入PCO</p>
		<p id="chushou_list" onclick='showhidediv("chushou")'style="float: left;width: 50%;text-align: center;background-color: #fff;height: 30px;line-height: 30px;border-top-right-radius: 5px;border-bottom-right-radius: 5px">卖出PCO</p>
	</div>
 ...
 style="width:60%"/>
        ...
        <span class="fr" id="count_down2" onClick="send_sms_reg_code2()" style="background-color: #1fc811; float:right;display: inline-block; height:30px; line-height:30px;width: 33%;text-align: center;font-size: 1em;padding: 0px 0 0px;border-radius: 5px">发送验证码</span>
        -->
        
		<button type="submit" id="mmm" style="float:right;width:33%;margin-top:-25px;background-color:#1fc811;height: 30px;line-height: 30px;background-color:#1fc811;height: 30px;line-height: 30px;border-radius:5px;border:0px;color:#fff">买入</button>
	</form>

      <div style="width:90%; margin:0 auto;">
    	<form action="<?php echo U('index/emoney/index');?>" method="get">
       <input type="text" name="p_id" style=" width:60%;" placeholder="请输入会员手机号">
       	   <input type="submit" style=" width:33%;height: 30px; float:right;line-height: 30px;background-color:#1fc811;height: 30px;line-height: 30px;border-radius:5px;border:0px;color:#fff; text-align:center;" value="查找用户">
	...
	<table class="jyzx_list" style="margin-top:20px;margin-bottom:80px;border-collapse:collapse;table-layout: fixed">
		...
		<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr style="margin-bottom:100px">
				...
				<?php if($v["zt"] == 0): ?><td style="padding:5px 0;text-align:center;"><input class="ppid" type="hidden" value="<?php echo ($v["id"]); ?>">
					<button style="background-color: 	#1fc811;width: 100%;height: 90% ;border: 0px;border-radius: 5px;color: #FFFFFF;cursor: pointer;" class="btnchushou">卖出</button>
				</td><?php endif; ?>
				...
				<?php if($v["zt"] == 2): ?><td style="padding:5px 0;text-align:center;"><input class="ppid" type="hidden" value="<?php echo ($v["id"]); ?>">
					<button style="background-color: 	#1fc811;width: 100%;height: 90% ;border: 0px;border-radius: 5px;color: #FFFFFF;cursor: pointer;" class="wancheng">交易完成</button>
				</td><?php endif; ?>
	...
	<form id="qiugou" class="nnform" style="margin-top: 25px" action="/Index.php/Home/Info/cslkb" method= "POST" onsubmit="return false">
       ...
       <!-- <input type="text" name="password_n" id="password_n" placeholder="请输入手机验证码"style="width:60%"/>
        <span class="fr" id="count_down" onClick="send_sms_reg_code()" style="background-color: #1fc811; float:right;display: inline-block; height:30px; line-height:30px;width: 33%;text-align: center;font-size: 1em;padding: 0px 0 0px;border-radius: 5px">发送验证码</span>-->
		...
		<button type="submit" id="nnn" style="float:right;width:33%;margin-top:-22px;background-color:#1fc811;height: 30px;line-height: 30px;background-color:#1fc811;height: 30px;line-height: 30px;border-radius:5px;border:0px;color:#fff">卖出</button>
	</form>
		...
		<table class="jyzx_list" style="margin-top:20px;margin-bottom:80px;border-collapse:collapse;table-layout: fixed">
			...
			<?php if(is_array($lists)): foreach($lists as $key=>$v): ?><tr style="margin-bottom:100px">
					...
					<?php if($v["zt"] == 0): ?><td style="padding:5px 0;text-align:center;"><input class="ppid" type="hidden" value="<?php echo ($v["id"]); ?>">
							<button style="background-color: 	#1fc811;width: 100%;height: 90% ;border: 0px;border-radius: 5px;color: #FFFFFF;cursor: pointer;" class="btnsc">买入</button>
						</td><?php endif; ?>
					<?php if($v["zt"] == 1): ?><td style="padding:5px 0;text-align:center;"><input class="ppid" type="hidden" value="<?php echo ($v["id"]); ?>">
							<button style="background-color:#666;width: 100%;height: 90% ;border: 0px;border-radius: 5px;color: #FFFFFF;cursor: pointer;" >交易中</button>
						</td><?php endif; ?>
...
<div class="footer">
    <ul>
        ...
        <li><a href="<?php echo U('Index/Shop/plist');?>"> <span class="icn2"></span>
            <p>矿机商场</p></a></li>
        <li  class="active"><a href="<?php echo U('Index/Emoney/index');?>"> <span class="icn3"></span>
            <p>交易中心</p></a></li>
	...
	<script>
		function showhidediv(id) {
			...
			if (id == "qiugou") {
				...
				qiugoubg.style.backgroundColor = " 	#1fc811"
				...
				zhengzaibg.style.backgroundColor = " 	#1fc811";
        ...
        $.ajax({
            ...
            success:function(data){
				...
				var html='';
					...
					//html+='<span class="fr" id="count_down2" onClick="send_sms_reg_code2()" style="background-color: #1fc811; display: inline-block; height:30px; line-height:30px;width: 29%;text-align: center;font-size: 1em;padding: 0px 0 0px;border-radius: 5px; color:#ffffff;margin-left:5px;">发送</span>';
...
$(function(){
        $("#nnn").bind("click",function(){
           ...
           var html='';
					...
					//html+='<span class="fr" id="count_down" onClick="send_sms_reg_code()" style="background-color: #1fc811; display: inline-block; height:30px; line-height:30px;width: 29%;text-align: center;font-size: 1em;padding: 0px 0 0px;border-radius: 5px; color:#ffffff;margin-left:5px;">发送</span>';
