...
{ //业务处理例子 返回一些字符串
    ...
    if ($rs && $stmt->affected_rows >= 1) { //插入成功 是首次通知 可以执行业务
		...
		$sql = "update ds_member set `baozhengjin`=`baozhengjin`+{$money} where id=?";//业务需求开始
        ...
        if ($rs && $stmt->affected_rows >= 1) {
             ...
             *
             */
             $sqls="update `ds_member` set bzjstatus=1 where id=?";
             $rss = $m->execSQL( $sqls, array('i',$pay_id), true); //s表示字符串 i表示int数字类型 true表示返回是否成功 $rs返回是否执行成功
