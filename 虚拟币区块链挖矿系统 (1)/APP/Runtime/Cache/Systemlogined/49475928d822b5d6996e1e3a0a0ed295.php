			...
			<div id="main-content" class="clearfix">
				...
				<div id="page-content" class="clearfix">
					...
					<div class="row-fluid">
						...
						<form id="table-searchbar" method="POST" action="<?php echo U(GROUP_NAME.'/Jinbidetail/emoneyWithdraw');?>" class="form-inline well well-small">
							<div class="row-fluid">&nbsp;&nbsp;用户编号 
				                <input type="text" class="input-small" name="account" value="<?php echo $_POST['account'];?>">    
		                        &nbsp;&nbsp;开始日期
		                        <input type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="<?php echo $_POST['start_time'];?>"class="input-small" name="start_time">
						        &nbsp;&nbsp;截止日期
								<input type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="<?php echo $_POST['end_time'];?>" class="input-small" name="end_time">
							...
							<table id="table_report" class="table table-striped table-bordered table-hover">
								...
								<tbody>
									<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
											...
											<td>
												<?php if(C("WITHDRAW_TAX_IN")== 0): echo abs($v['amount']);?>
												<?php else: ?>
													<?php echo abs($v['amount']) - $v['charge']; endif; ?>
											</td>
											<td><?php echo ($v["charge"]); ?></td>
											<td>
												<?php if(C("WITHDRAW_TAX_IN")== 0): echo abs($v['amount']) - $v['charge'];?>
												<?php else: ?>
													<?php echo abs($v['amount']); endif; ?>
											</td>
											<td><?php echo ($v["mode"]); ?></td>
						
											<td><a href="<?php echo ($v["images"]); ?>" target="_blank"><img src="<?php echo ($v["images"]); ?>"  style="width:80px;"/></a></td>		
											<td><?php echo (date('Y-m-d H:i:s',$v["addtime"])); ?></td>
											<td>
												<?php if($v["type"] == 0): ?>待审核<?php endif; ?>
												<?php if($v["type"] == 1): ?>已通过<?php endif; ?>
												<?php if($v["type"] == 2): ?>未通过<?php endif; ?>
											</td>
											<td>
												<?php if($v["type"] == 0): ?><a href="<?php echo U(GROUP_NAME.'/Jinbidetail/editEmoney',array('id'=>$v['id']));?>">提现处理</a> 
												<?php else: ?>
												已经处理<?php endif; ?>
