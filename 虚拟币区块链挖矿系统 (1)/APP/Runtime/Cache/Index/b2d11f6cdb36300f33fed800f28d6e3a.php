    ...
    <header class="header">
		...
		<?php else: ?>
			...
			<?php if(($member['shopstatus'] == 0) or ($member['shopstatus'] == 1)): if(!empty($member['shopname'])): ?><div class="pic">店铺名称：&nbsp;&nbsp;&nbsp;<?php echo ($member['shopname']); ?></div>
               		 <?php else: ?>
				<div class="pic">店铺名称：<input type="text" placeholder="请输入店铺名称" name="shopname" style="height: 30px;line-height: 30px;width: 69%;border-radius: 5px;border:none;color:#eee;padding-left: 5px;-webkit-appearance:none;" /></div><?php endif; ?> 
				...
				<select name="shoplevel" style="height: 30px;line-height: 30px;width: 69%;border-radius: 5px;border:none;color:#eee; background:#ff7387;padding-left: 5px;-webkit-appearance:none;" />
						<option value="">请选择商户级别</option>
							<?php if(is_array($shop_group)): foreach($shop_group as $key=>$v): ?><option value="<?php echo ($v["level"]); ?>"><?php echo ($v["price"]); ?>个PCO<?php echo ($v["name"]); ?>可以上传<?php echo ($v["goosnum"]); ?>件商品</option><?php endforeach; endif; ?>
                </select>
				</div>
				<div class="pic">支付密码：<input type="text" placeholder="请输入支付密码" name="password2" style="height: 30px;line-height: 30px;width: 69%;border-radius: 5px;border:none;color:#eee; padding-left: 5px;-webkit-appearance:none;" /></div><?php endif; endif; ?>
		<?php if(empty($member['shoplevel']) || empty($member['shopname'])): ?><button type="submit" class="botton" style="width: 100%;height: 30px;line-height: 30px;border-radius: 5px;border: 0px; background-color:#ff7387;margin-top: 5px;color: #FFFFFF;-webkit-appearance: none;">提 交</button><?php endif; ?>
		</form>	
