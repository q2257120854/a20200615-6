...
if($row1=mysql_fetch_array($res1)){$pre="<a href='ggview".$row1[id].".html'>".$row1[tit]."</a>";}else{$pre="已是第一篇公告";}

while1("*","yjcode_gg where sj<='".$row[sj]."' and id<>".$row[id]." and zt<>99 order by sj desc");
if($row1=mysql_fetch_array($res1)){$nex="<a href='ggview".$row1[id].".html'>".$row1[tit]."</a>";}else{$nex="已是最后一篇公告";}
?>
<html>
<head>
...
<div class="helpright">
 ...
 <a href="gglist.html">网站公告</a> <span>>></span> 
 <?=$row[tit]?>
 </div>
