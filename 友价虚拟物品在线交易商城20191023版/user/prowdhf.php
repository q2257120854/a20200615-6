...
while0("*","yjcode_wenda where selluserid=".$userid." and id=".$id);if(!$row=mysql_fetch_array($res)){Audit_alert("来源错误","prowdlist.php","parent.");}

if($_GET[control]=="update"){
 zwzr();
 updatetable("yjcode_wenda","hftxt='".sqlzhuru($_POST[s1])."',hfsj='".$sj."' where selluserid=".$userid." and id=".$id);
 php_toheader("prowdhf.php?t=suc&id=".$id);
}
?>
...
f1.action="prowdhf.php?control=update&id=<?=$id?>";
}
<? if($_GET["t"]=="suc"){?>
...
<ul class="uk">
<li class="l1">回复内容：</li>
<li class="l2"><textarea name="s1"><?=$row[hftxt]?></textarea></li>
