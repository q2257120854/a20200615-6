...
while0("*","yjcode_pro where bh='".$bh."' and userid=".$userid);if(!$row=mysql_fetch_array($res)){exit;}

if($_GET[control]=="add"){
 $area="|".sqlzhuru($_POST[area1]).",".sqlzhuru($_POST[add2]).",".sqlzhuru($_POST[add3])."|";
 if(strstr($row[ysarea],$area)==""){$areav=$row[ysarea].$area;updatetable("yjcode_pro","ysarea='".$areav."' where id=".$row[id]." and userid=".$userid);}
 ...
 updatetable("yjcode_pro","ysarea='".$str."' where id=".$row[id]." and userid=".$userid);
 php_toheader("ysarea.php?bh=".$bh);
