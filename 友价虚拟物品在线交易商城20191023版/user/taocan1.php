...
$ty1id=$_GET[ty1id];
while0("*","yjcode_taocan where userid=".$userid." and id=".$ty1id);$row=mysql_fetch_array($res);
$ty1tit=$row[tit];
...
if($_GET[control]=="update"){
 zwzr();
 if(panduan("*","yjcode_taocan where userid=".$userid." and admin=2 and tit='".sqlzhuru($_POST[t0])."' and tit2='".sqlzhuru($_POST[t1])."' and probh='".$bh."' and id<>".$id)==1){Audit_alert("该套餐说明已存在！","taocan1.php?action=update&id=".$id."&ty1id=".$_GET[ty1id]."&bh=".$bh);}
 ...
 php_toheader("taocan1.php?id=".$id."&ty1id=".$ty1id."&bh=".$bh);
...
<div class="userright">
 ...
 <li class="l1">套餐图标：</li>
 <li class="l2"><span class="finp"><input type="file" name="inp2" id="inp2" size="15" accept=".jpg,.gif,.jpeg,.png"> 最佳尺寸：350*350,不上传则显示文字形式</span></li>
