...
$sj=date("Y-m-d H:i:s");
$pid=$_GET[pid];
while0("*","yjcode_city where bh='".$pid."' and level=2");if(!$row=mysql_fetch_array($res)){php_toheader("citylist.php");}
$city2=$row[name1];
while0("*","yjcode_city where bh='".$row[parentid]."' and level=1");if(!$row=mysql_fetch_array($res)){php_toheader("citylist.php");}
$mbh=$row[bh];
$city1=$row[name1];
...
<div class="right">
 ...
 while0("*","yjcode_city where level=3 and id=".$_GET[id]);if(!$row=mysql_fetch_array($res)){php_toheader("citylist.php");}
 ?>
 <script language="javascript">
 function tj(){
 if((document.f1.t1.value).replace(/\s/,"")==""){alert("请输入名称！");document.f1.t1.focus();return false;}
