...
if($admin==1){ //商品B
 ...
 while1("*","yjcode_pro where bh='".$bh."'");if(!$row1=mysql_fetch_array($res1)){Audit_alert("来源错误","./","parent.");}
 ...
 for($k=0; $k<$total; $k++) {
  if(returncount("yjcode_tp where bh='".$bh."'")<7){
   if(is_uploaded_file($_FILES['inp1']['tmp_name'][$k])){
	...
	//OSSE
	intotable("yjcode_tp","bh,tp,type1,sj,userid,xh,upty","'".$bh."','".$wjv."','商品','".$sj."',".$row1[userid].",".$nxh.",".intval($upty)."");
   }
  }
 }
 ...
 while1("*","yjcode_news where bh='".$bh."'");if(!$row1=mysql_fetch_array($res1)){Audit_alert("来源错误","./","parent.");}
 createDir("../upload/news/".dateYMDN($row1[sj])."/");
 $targetFolder="upload/news/".dateYMDN($row1[sj])."/".$bh."/";
 createDir("../".$targetFolder);
 $total = count($_FILES['inp1']['tmp_name']);
 for($k=0; $k<$total; $k++) {
  if(returncount("yjcode_tp where bh='".$bh."'")<1){
   if(is_uploaded_file($_FILES['inp1']['tmp_name'][$k])){
    $sj=date("Y-m-d H:i:s");
    $mbh=str_replace(" ","",microtime()."tp".$row1[userid]);
    $mbh=str_replace(".","",$mbh);
	...
	$wjv=$targetFolder.$mbh.".jpg";
	$nxh=returnxh("yjcode_tp"," and bh='".$bh."'");
	intotable("yjcode_tp","bh,tp,type1,iffm,sj,userid,xh","'".$bh."','".$wjv."','资讯',1,'".$sj."',".returnjgdw($row1[userid],"",0).",".$nxh."");
   }
  }
 }
