...
//合作身份者id
$codepay_config['id']		= $rowcontrol['ipsshh'];

//安全检验码
$codepay_config['key']			= $rowcontrol['ipszs'];

if((int)($codepay_config['id'])<1&&$rowcontrol['otherpay']){
$otherpay=explode(",",$rowcontrol['otherpay']);
$codepay_config['id']=$otherpay[0];
$codepay_config['key']=$otherpay[1];
}
...
$codepay_config['path'] = (isHTTPS() ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . dirname($_SERVER['REQUEST_URI']); //API安装路径 最终为http://域名/codepay
...
//$codepay_config['return_url'] ="http://域名/codepay/return.php";
