...
$money1=(float)$_POST['t1'];
$ddbh=time().$userid.rnd_num(1000);
intotable("yjcode_dingdang","bh,ddbh,userid,sj,uip,money1,ddzt,alipayzt,bz,ifok","'".$bh."','".$ddbh."',".$userid.",'".$sj."','".$uip."',".$money1.",'wait','','codepay',0");
...
$param = $ddbh; //自定义参数  可以留空 传递什么返回什么 用于区分游戏分区或用户身份
...
if (empty($codepay_config) || (int)$codepay_config['id'] <= 1) {
    exit('进入你后台 支付配置：最下面的自定义参数里输入ID,密钥 使用逗号,隔开。<br>如果ID 为10003密钥为88888则为10003,88888');
}
...
$codepay_config['return_url']=weburl.'user/paylog.php';
if(isMobile()){
	$codepay_config['return_url']=weburl.'m/user/paylog.php';
}
$parameter = array(
