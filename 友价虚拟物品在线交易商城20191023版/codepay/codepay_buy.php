...
$sqluser = "select * from yjcode_user where uid='" . $_SESSION[SHOPUSER] . "'";
mysql_query("SET NAMES 'GBK'");
$resuser = mysql_query($sqluser);
if (!$rowuser = mysql_fetch_array($resuser)) {
    php_toheader("../user/reg/");
}
...
if (sqlzhuru($_POST[jvs]) == "carpay") {
    if ($needmoney <= $usermoney) {
        Audit_alert("您的可用余额充足，请用余额直接支付。", "../user/carpay.php?carid=" . $carid);
    }
    zwzr();
    $bh = time();
    $BillNo = date('YmdHis') . mt_rand(100000, 999999) . "|" . $rowuser[id];
    $money1 = ($needmoney * 10 - $usermoney * 10) / 10;
    $back = intotable("yjcode_dingdang", "bh,ddbh,userid,sj,uip,money1,ddzt,alipayzt,bz,ifok,carid", "'" . $bh . "','" . $BillNo . "'," . $rowuser[id] . ",'" . $sj . "','" . $uip . "'," . $money1 . ",'wait','','codepay',0,'" . ((int)$caridarr) . "'");
}
...
$param = $BillNo; //自定义参数  可以留空 传递什么返回什么 用于区分游戏分区或用户身份

$type = (int)$_POST["R1"]; //支付方式
 ...
 */
//构造要请求的参数数组，无需改动
$codepay_config['return_url']= weburl . 'user/order.php';
function isMobile(){   
    if(strpos($_SERVER["HTTP_REFERER"],"/m/")>1){
    return true; 
}
...
$codepay_config['return_url']=weburl.'user/order.php';
if(isMobile()){
	$codepay_config['return_url']=weburl.'m/user/order.php';
}
$parameter = array(
    ...
    "notify_url" => weburl . "codepay/notify_buy.php",//付款后通知该页面处理业务
