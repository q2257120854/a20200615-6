...
function updatePageidl() {
 if (xmlHttpidl.readyState == 4) {
  ...
  a=response.split("|");
  if(a[0]=="err1"){idlzt(1);alert("帐号密码有误");return false;}
  else if(a[0]=="err2"){return false;}
  else if(a[0]=="ok"){
  userCheckses();
  ...
  }else{idlzt(1);return false;}
 }
}
function idlzt(x){
if(x==0){
document.getElementById("idl").style.display="none";
document.getElementById("idling").style.display="";
}else{
document.getElementById("idl").style.display="";
document.getElementById("idling").style.display="none";
}
}
