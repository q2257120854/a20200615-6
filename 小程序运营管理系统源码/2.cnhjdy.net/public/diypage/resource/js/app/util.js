	...
	util.iconBrowser = function(a) {
		require(["fontawesome"], function() {
			...
			c.modal({
				...
				height: "70%",
				"overflow-y": "scroll"
			}), c.modal("show"), window.selectIconComplete = function(b) {
		...
		require(["emoji"], function() {
			...
			c.modal({
				...
				height: "70%",
				"overflow-y": "scroll"
			}), c.modal("show"), window.selectEmojiComplete = function(b) {
				$.isFunction(a) && (a(b), c.modal("hide"))
			}
		})
		...
		c.modal({
			...
			height: "300px",
			"overflow-y": "auto"
		}), c.modal("show"), window.selectLinkComplete = function(b) {
			$.isFunction(a) && (a(b), c.modal("hide"))
		}
		...
		d.modal({
			...
			height: "700px",
			"overflow-y": "auto"
		}), d.modal("show"), window.pageLinkComplete = function(b, c) {
		...
		d.modal({
			...
			height: "700px",
			"overflow-y": "auto"
		}), d.modal("show"), window.newsLinkComplete = function(b, c) {
			$.isFunction(a) && (a(b, c), "" != c && void 0 != c || d.modal("hide"))
		}
		...
		d.modal({
			...
			height: "700px",
			"overflow-y": "auto"
		}), d.modal("show"), window.articleLinkComplete = function(b, c) {
			$.isFunction(a) && (a(b, c), "" != c && void 0 != c || d.modal("hide"))
		}
		...
		d.modal({
			...
			height: "700px",
			"overflow-y": "auto"
		}), d.modal("show"), window.phoneLinkComplete = function(b, c) {
			$.isFunction(a) && (a(b, c), "" != c && void 0 != c || d.modal("hide"))
		}
		...
		b.modal({
			...
			height: "700px",
			"overflow-y": "auto"
		}), b.modal("show"), window.moduleLinkComplete = function(c, d) {
		...
		require(["clipboard"], function(c) {
			...
			d.on("success", function(a) {
				util.message("复制成功", "", "success"), a.clearSelection()
		...
		c = $.extend({}, c, b), require(["fileUploader"], function(b) {
			b.show(function(b) {
				if(b.length > 0) {
		...
		$.isFunction(b) && (b = {
			callback: b
		}), b = $.extend({}, d, b), window.UEDITOR_HOME_URL = window.sysinfo.siteroot + "web/resource/components/ueditor/";
		var e = function(d, e) {
			var f = {
			...
			if(d.registerUI("myinsertimage", function(a, b) {
					a.registerCommand(b, {
				...
				}, 19), d.registerUI("myinsertvideo", function(a, c) {
					a.registerCommand(c, {
						execCommand: function() {
							e.show(function(b) {
								if(b) {
				...
				$("#" + c).removeClass("form-control"), $("#" + c).data("editor", h), $("#" + c).parents("form").submit(function() {
					h.queryCommandState("source") && h.execCommand("source")
				}), $.isFunction(b.callback) && b.callback(a, h)
			}
		};
		require(["ueditor", "fileUploader"], function(a, b) {
			e(a, b)
		}, function(a) {
		...
		b || c || (c = "info"), -1 == $.inArray(c, ["success", "error", "info", "warning"]) && (c = ""), "" == c && (c = "" == b ? "error" : "success");
		var d = {
				success: "right-sign",
				error: "error-sign",
				danger: "error-sign",
		...
		if(b && b.length > 0) {
			if("success" == c) {
				var f = new Object;
				return f.type = c, f.msg = a, util.cookie.set("message", JSON.stringify(f), 600), "back" == b ? window.history.back(-1) : "refresh" == b ? (b = location.href, window.location.href = b) : window.location.href = b
			}
		...
		return b && b.length > 0 && "success" != c && i.on("hidden.bs.modal", function() {
		...
		if(!c || getQuery(c) == getQuery(window.location.href)) {
			var g = {
				...
				h = !1,
				i = "";
			d || (d = "info"), -1 == $.inArray(d, ["success", "error", "info", "warning", "danger"]) && (d = ""), "" == d && (d = "success"), -1 != $.inArray(d, ["success"]) && (h = !0, e = e ? e : 3);
			...
			return h ? (l.modal({
				backdrop: !1
			}), l.addClass("modal-" + d), l.on("show.bs.modal", function() {
				f()
		...
		if(0 == f.length) {
			...
			}), f.find(".modal-dialog").css("width", "80%"), f.modal({
				keyboard: !1
			}), map = util.map.instance = new BMap.Map("map-container"), map.centerAndZoom(d, 12), map.enableScrollWheelZoom(), map.enableDragging(), map.enableContinuousZoom(), map.addControl(new BMap.NavigationControl), map.addControl(new BMap.OverviewMapControl), marker = util.map.marker = new BMap.Marker(d), marker.setLabel(new BMap.Label("请您移动此标记，选择您的坐标！", {
				offset: new BMap.Size(10, -20)
			})), map.addOverlay(marker), marker.enableDragging(), marker.addEventListener("dragend", function(a) {
				var b = marker.getPosition();
				e.getLocation(b, function(a) {
		...
		f.off("shown.bs.modal"), f.on("shown.bs.modal", function() {
			marker.setPosition(d), map.panTo(marker.getPosition())
		}), f.find("button.btn-primary").off("click"), f.find("button.btn-primary").on("click", function() {
			if($.isFunction(b)) {
				var a = util.map.marker.getPosition();
				e.getLocation(a, function(c) {
					var d = {
						lng: a.lng,
						lat: a.lat,
						label: c.address
		...
		!c && d && (c = d), e = $.extend({}, e, c), e.type = "image", require(["http://wxkf2.nttrip.cn/web/resource/components/fileuploader/fileuploader.min.js"], function(a) {
			a.show(function(a) {
				a && $.isFunction(b) && b(a)
			}, e)
		})
		...
		d = $.extend({}, d, c), require(["fileUploader"], function(a) {
			a.show(function(a) {
				a && $.isFunction(b) && b(a)
			}, d)
		})
		...
		a && (e.path = a), !c && d && (c = d), e = $.extend({}, e, c), require(["fileUploader"], function(a) {
			a.show(function(a) {
				a && $.isFunction(b) && b(a)
			}, e)
		})
		...
		a && (d.path = a), d = $.extend({}, d, c), require(["fileUploader"], function(a) {
			a.show(function(a) {
				a && $.isFunction(b) && b(a)
			}, d)
		})
		...
		if("undeinfed" != typeof g.width && g.width > 0 && j.find(".modal-dialog").css({
			...
			for(i in d) "function" == typeof d[i] && j.on(i, d[i]);
		var k;
		return j.find(".modal-body").load(a, function(a) {
			try {
				k = $.parseJSON(a), j.find(".modal-body").html('<div class="modal-body"><i class="pull-left fa fa-4x ' + (k.message.errno ? "fa-info-circle" : "fa-check-circle") + '"></i><div class="pull-left"><p>' + k.message.message + '</p></div><div class="clearfix"></div></div>')
			} catch(b) {
		...
		}), "function" == typeof d.confirm && j.find(".confirm", j).on("click", d.confirm), j.modal(g)
	}, util.cookie = {
		...
		c = $.extend({}, c, b), require(["coupon"], function(b) {
			b.init(function(b) {
				b && $.isFunction(a) && a(b)
			}, c)
		})
		...
		c = $.extend({}, c, b), require(["material"], function(b) {
			b.init(function(b) {
				b && $.isFunction(a) && a(b)
			}, c)
		})
		...
		if(a = $.trim(a), "string" == typeof a && a.length > 3) {
			for(var b = /^./, c = b.exec(a), b = /.$/, d = b.exec(a)[0], e = "", f = 0; f < a.length - 2; f++) e += "*";
			return a = c + e + d
		}
		return a
