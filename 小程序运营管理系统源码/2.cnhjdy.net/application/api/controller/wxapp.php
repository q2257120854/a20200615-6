...
class Sudu8_pageModuleWxapp extends WeModuleWxapp {
	...
	public function doPagegetfoot(){
		...
		$foot = $_GPC['foot'];
		if($foot==1){
			$baseInfo = pdo_fetch("SELECT tabnum_new,color_bar,tabbar_bg,tabbar_tc,tabbar_tca,tabbar_new,tabbar_t FROM ".tablename('sudu8_page_base')." WHERE uniacid = :uniacid" , array(':uniacid' => $uniacid));
			$baseInfo['tabbar'] = unserialize($baseInfo['tabbar_new']);
			$baseInfo['tabnum'] = $baseInfo['tabnum_new'];
			for ($i=0; $i<=4; $i++) {
				$baseInfo['tabbar'][$i] = unserialize($baseInfo['tabbar'][$i]);
...
// var_dump(11111);exit;
        ...
        if($types == 'duo'){
        	...
        	if($price == $gpc_price && $price > $yue && round($price - $yue, 2).'' == $payprice.''){
        		...
        		if($order['coupon'] != 0){
					...
					if($coupon['flag'] == 1){
        				$this->checkthisyhq($order_id, $order['price'], $couponinfo['price'], $types);
        		...
        		$weixinpay = $this->getweixinpayinfo($openid, $order_id, $payprice, $types."|".$_GPC['formId']);  //最后一个参数为附加参数，这里存订单类型+formId
        		$weixinpay['err'] = 0;
	...
	public function dopagepaynotify(){
        ...
        if($types == 'duo'){
        	...
        	$this->sendTplMessage($flag, $openid, $formId, 'duo_zf', $data8);
	...
	public function doPagepayoverFxs(){
		...
		$order = pdo_get("sudu8_page_duo_products_order", array("uniacid"=>$uniacid, "order_id"=>$order_id));
		$jsondata = unserialize($order['jsondata']);
		$product = pdo_get("sudu8_page_products", array("uniacid"=>$uniacid, "id"=>$jsondata['baseinfo']['id']));
		...
		if($product['fx_uni'] == 1){
			if($product['commission_type'] == 1){
				$onemoney = round($price * $product['commission_one'] / 100, 2);
				$twomoney = round($price * $product['commission_two'] / 100, 2);
				$threemoney = round($price * $product['commission_three'] / 100, 2);
			}
			if($product['commission_type'] == 2){
				$onemoney = $product['commission_one'];
				$twomoney = $product['commission_two'];
				$threemoney = $product['commission_three'];
			}
		}else{
			$onemoney = round($price * $guiz['one_bili'] / 100, 2);
			$twomoney = round($price * $guiz['two_bili'] / 100, 2);
			$threemoney = round($price * $guiz['three_bili'] / 100, 2);
		}
	...
	public function sendMailToAdmin($order_id){
		require_once(IA_ROOT."/framework/library/phpmailer/class.phpmailer.php");
		require_once(IA_ROOT."/framework/library/phpmailer/class.smtp.php");
		global $_GPC, $_W;
		$uniacid = $_W['uniacid'];
	...
	public function doPagerefundnotify(){
		...
		if($types == 'duo' || $types == 'duoqx'){
	        ...
	        if($order['coupon']){
	            pdo_update("sudu8_page_coupon_user", array("uniacid"=>$uniacid, "uid"=>$order['uid'], "id"=>$order['coupon']));
	        }
	        if($order['jf']){
	        ...
	        // $flag = 6;
	        // $this->sendTplMessage($flag, $order['openid'], $order['prepayid'], 'duo_th', array("orderid"=>$order['order_id'],"fprice"=>$order['price']));
		}
	        ...
	        foreach($data['items'] as $k => $v){
	        	if(is_array($v)){
		        	...
		        	if($v['id'] == "msmk"){
		        		...
		        		$sql = "SELECT title,thumb,id,`desc`,price,market_price,sale_num,sale_tnum,sale_end_time,pro_kc FROM ".tablename('sudu8_page_products')." WHERE `uniacid` = {$_W['uniacid']} AND `type` = 'showPro' AND `is_more` = 0 AND `cid` = {$sourceid} or `pcid` = {$sourceid}  ".$where." LIMIT 0,{$count}";
		        		$list = pdo_fetchall($sql);
		        		$djssql = "SELECT sale_end_time FROM ".tablename('sudu8_page_products')." WHERE `uniacid` = {$_W['uniacid']} AND `type` = 'showPro' AND `is_more` = 0 AND `cid` = {$sourceid} ".$where." LIMIT 0,{$count}";
		        	...
		        	if($v['id'] == "pt"){
		        		...
		        		$sql = "SELECT * FROM ".tablename('sudu8_page_pt_pro')." WHERE `uniacid` = {$_W['uniacid']}  AND `cid` = {$sourceid} ".$where." LIMIT 0,{$count}";
		        		$list = pdo_fetchall($sql);
		        		if($list){
		        			foreach($list as $kk => $vv){
		        	...
		        	if($v['id'] == "cases"){
		        		...
		        		$sql = "SELECT id,title,thumb,type FROM ".tablename('sudu8_page_products')." WHERE (`type` = 'showPic' or `type` = 'showArt') AND `uniacid` = {$_W['uniacid']}  AND `cid` = {$sourceid} or `pcid` = {$sourceid} ".$where." LIMIT 0,{$count}";
		        		$list = pdo_fetchall($sql);
		        		if($list){
		        			foreach($list as $kk => $vv){
		        	...
		        	if($v['id'] == "listdesc"){
		        		...
		        		$sql = "SELECT * FROM ".tablename('sudu8_page_products')." WHERE `type` = 'showArt' AND  `uniacid` = {$_W['uniacid']}  AND `cid` = {$sourceid} or `pcid` = {$sourceid} ".$where." LIMIT 0,{$count}";
		        		$list = pdo_fetchall($sql);
		        		if($list){
		        			foreach($list as $kk => $vv){
		        	...
		        	if(isset($v['params']['noticedata']) && intval($v['params']['noticedata']) == 0){
		        		...
		        		$sql = "SELECT id,title FROM ".tablename('sudu8_page_products')." WHERE `uniacid` = {$_W['uniacid']} AND `type` = 'showArt' AND `cid` = {$sourceid} or `pcid` = {$sourceid} ORDER BY id DESC LIMIT 0,{$count}";
		        		$list = pdo_fetchall($sql);
		        		if($list){
		        			foreach($list as $kk => $vv){
		        	...
		        	if($v['id'] == "goods"){
		        		...
		        		$sql = "SELECT * FROM ".tablename('sudu8_page_products')." WHERE `uniacid` = {$_W['uniacid']}  AND `cid` = {$sourceid} or `pcid` = {$sourceid} ".$where." LIMIT 0,{$count}";
		        		$list = pdo_fetchall($sql);
