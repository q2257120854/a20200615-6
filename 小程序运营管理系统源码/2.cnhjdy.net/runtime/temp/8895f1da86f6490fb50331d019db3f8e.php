...
<input type="hidden" id="nowhtml" value="navModel" class="navModel5-4">
<ul class="breadcrumb">
    <li>
        <i class="icon-home"></i>
        订单管理--<?php echo $applet['name']; ?>
    </li>
</ul>
<style type="text/css">
    .shuk{
...
<script type="text/javascript">
    ...
    function tijiao(){
        ...
        location.href = "<?php echo Url('Pt/order'); ?>?appletid=<?php echo $_GET['appletid']?>&op=fahuo&orderid="+order+"&kuadi="+kuaidi+"&kuaidihao="+kuaidihao;
    }
</script>
    <div class="row-fluid">
        <div class="span12">
            <div class="portlet box ">
                <div class="portlet-body">
                    ...
                    <div class="input-box" style="margin-bottom: 10px; position: relative; text-align:center; margin-top:20px; margin-bottom:40px; ">
                        请输入订单号：
                        <input type="text" placeholder="请输入搜索订单号" class="shuk" id="order_id" value="<?php echo $order_id; ?>">
        ...
        <script type="text/javascript">
            ...
            function xiaofei(id){
                if(window.confirm('确定核销该订单？')){
                    location.href = "<?php echo Url('Pt/order'); ?>?appletid=<?php echo $_GET['appletid']?>&op=hx&orderid="+id;
                }else{
                    return false;
                }
            }
            function fahuo(id){
                if(window.confirm('确定要结束该订单？')){          
                    location.href = "<?php echo Url('Pt/order'); ?>?appletid=<?php echo $_GET['appletid']?>&op=fh&orderid="+id;
                }else{
                    return false;
                }
            }
 ...
 -->
    ...
    <thead>
        <tr style="background:#f8f8f8">
            ...
            <td style="width:100px;text-align: center;">核销信息</td>
...
<?php foreach($orders as $item): ?>
    <tr class="trorder">
        <td colspan="3" style="border-left:1px solid #dedede;">
            订单编号:  <?php echo $item['order_id']; ?>                               
        </td>
        <td colspan="2" style="border-left:1px solid #dedede;">
            拼团编号:  <?php echo $item['pt_order']; ?>
        </td>
        <td colspan="2" style="text-align:right;font-size:12px;border-right:1px solid #dedede" class="aops">
            <span style="float: right;">下单时间：<?php echo $item['creattime']; ?> </span>                        
        </td>
    </tr>

    <?php foreach($item['jsondata'] as $index => $val): ?>

    <tr class="trbody">
        <td style="overflow:hidden;border-left:1px solid #dedede;" >
            <img src="<?php echo $val['baseinfo2']['thumb']; ?>" style="width:50px;height:50px;border:1px solid #ccc; padding:1px;" onerror="">
        </td>
        <td style="text-align: left;overflow:hidden;border-left:none;">
            <?php if(isset($val['baseinfo2']['title'])): ?><?php echo $val['baseinfo2']['title']; endif; ?> <br> <?php echo $val['proinfo']['ggz']; ?>
        </td>
        ...
        <?php if($index == 0): ?>
        <td style="text-align:center;border-left:none;" rowspan="<?php echo $item['counts']; ?>">
            总价：<?php echo $item['allprice']; ?>
            <br/>
            实付：<?php echo $item['price']; ?>
        </td>
        <td style="text-align:right;border-left:none;" rowspan="<?php echo $item['counts']; ?>">
            <?php if($item['hxtime']!=0): ?><?php echo $item['hxtime']; endif; ?>
        </td>
        <td style="text-align:right;border-left:none;" rowspan="<?php echo $item['counts']; ?>">
            <?php echo $item['hxinfo2']; ?>
        </td>
        <td style="text-align:center;border-left:none;border-right:1px solid #dedede;" rowspan="<?php echo $item['counts']; ?>">
            <?php if($item['flag'] ==0 && $item['ck'] == 2 && $item['join_count'] < $item['pt_max'] && !$item['pt_tx']['flag'] || $item['flag'] == 0 && $item['ck'] == 1 && !$item['join_count'] && !$item['pt_tx']['flag']): ?><span class="btn btn-inverse btn-sm btn-disabled" style="background-color: #4a89dc;color: #fff">待支付</span><?php endif; if($item['flag'] ==0 && $item['ck'] == 2 && $item['join_count'] >= $item['pt_max'] && !$item['pt_tx']['flag'] || $item['flag'] ==3 && ($item['yue_price'] == 0 || $item['wx_price'] == 0) && !$item['pt_tx']['flag'] || $item['flag'] == 3 && $item['ck'] == 1 && !$item['join_count'] && !$item['pt_tx']['flag']): ?><span class="btn btn-inverse btn-sm btn-disabled" style="background-color: #4a89dc;color: #fff">待支付、已结束</span><?php endif; if($item['join_count'] < $item['pt_min'] && $item['flag'] == 1): ?>
            <span class="btn btn-inverse btn-sm btn-disabled" style="background-color: #4a89dc;color: #fff">已支付</span>
            <?php endif; if($item['flag'] ==1 && $item['nav'] == 2 && $item['join_count'] >= $item['pt_min'] && !$item['pt_tx']['flag'] || $item['flag'] ==3 && ($item['yue_price'] != 0 || $item['wx_price'] != 0) && $item['nav'] == 2 && $item['join_count'] >= $item['pt_min'] && !$item['pt_tx']['flag'] || $item['flag'] ==1 && $item['nav'] == 2 && $item['types'] == 2 && !$item['pt_tx']['flag']): ?>
            <a class="btn btn-danger btn-sm" onclick="xiaofei(<?php echo $item['id']; ?>)">立即核销</a>
            <?php endif; if($item['flag'] ==1 && $item['nav'] == 1 && $item['join_count'] >= $item['pt_min'] && !$item['pt_tx']['flag'] || $item['flag'] ==3 && ($item['yue_price'] != 0 || $item['wx_price'] != 0) && $item['nav'] == 1 && $item['join_count'] >= $item['pt_min'] && !$item['pt_tx']['flag'] || $item['flag'] ==1 && $item['nav'] == 1 && $item['types'] == 2 && !$item['pt_tx']['flag']): ?>
            <a class="btn btn-danger btn-sm" onclick="shoscc(<?php echo $item['id']; ?>)">立即发货</a>
            <?php endif; if($item['flag'] ==2): ?> <span class="btn btn-success btn-sm btn-disabled" style="background-color: #FC6E51;color: #fff">已完成</span><?php endif; if($item['flag'] ==4): ?> <a class="btn btn-success btn-sm btn-disabled" style="background-color: #ed5565;color: #fff">已发货</a><?php endif; if($item['pt_tx']['flag'] == 1): ?> <span class="btn btn-inverse btn-sm btn-disabled" style="background-color: #b5b5b5;color: #fff">待退款</span><?php endif; if($item['pt_tx']['flag'] == 2): ?> <span class="btn btn-inverse btn-sm btn-disabled" style="background-color: #b5b5b5;color: #fff">已退款</span><?php endif; if($item['pt_tx']['flag'] == 3): ?> <span class="btn btn-inverse btn-sm btn-disabled" style="background-color: #b5b5b5;color: #fff">已拒绝退款</span><?php endif; ?>
        </td>
        <?php endif; ?>
    ...
    <tr style="border-bottom:none;background:#f9f9f9;">
        ...
        <td colspan="4" style="text-align:left;border-left:1px solid #dedede;border-bottom:1px solid #dedede;" >
            <?php if($item['couponinfo']['price']): ?>
            优惠券：<?php echo $item['couponinfo']['price']; ?>元<span style="padding: 0 20px"></span>
            <?php endif; if($item['jf']>0): ?>
            积分：<?php echo $item['jf']; ?>【抵扣<?php echo $item['jfmoney']; ?>元】
            <?php endif; ?>
        </td>
        
        <td colspan="3" style="text-align:right;border-right:1px solid #dedede;border-bottom:1px solid #dedede;">
            买家留言： <?php echo $item['liuyan']; ?>
        </td>
    </tr>
    <tr>
        <td colspan="4" style="height:20px;padding:8px;border-top:none;border-left:1px solid #dedede;border-bottom:1px solid #dedede;">
            <?php if($item['address_get']): ?>
                姓名：<?php echo $item['address_get']['name']; ?><br/>
                电话：<?php echo $item['address_get']['mobile']; ?><br/>
                邮编：<?php echo $item['address_get']['postalcode']; ?><br/>
                地址：<?php echo $item['address_get']['address']; ?> <?php echo $item['address_get']['more_address']; ?><br/>
            <?php endif; ?>
        </td>
        <td colspan="3" style="height:20px;padding:8px;border-top:none;border-right:1px solid #dedede;border-bottom:1px solid #dedede;">
            <?php if($item['kuadi']): ?>
            快递：<?php echo $item['kuadi']; ?><br/>
            快递号：<?php echo $item['kuaidihao']; endif; ?>
        </td>
    </tr>
    <tr>
