...
<input type="hidden" id="nowhtml" value="navSale" class="navSale1-1">

	<ul class="breadcrumb">
		<li>
			<i class="icon-home"></i>
			优惠券管理--<?php echo $applet['name']; ?>
		</li>
	</ul>
			...
			<div class="portlet-body">
				...
				<div class="input-box" style="margin-bottom: 10px; position: relative;">
					...
					<div class="btn-group" style="float:right">
						...
						<a href="<?php echo Url('Coupon/add'); ?>?appletid=<?php echo $_GET['appletid']?>" >
						<button id="sample_editable_1_new" class="btn green">
						添加优惠券 <i class="icon-plus"></i>
						</button>
						</a>
				...
				<table class="table table-striped table-hover table-bordered" id="sample_editable_1">
					...
					<thead>
						...
						<tr>
							...
							<th style="width: 200px;">剩余库存</th>

							<th style="width: 150px;">限制领</th>
					...
					<?php if($coupon): foreach($coupon['data'] as $item): ?>
							<tr>
								<td>
									<?php echo $item['num']; ?>
								</td>
								...
								<td>
									<?php if($item['counts'] == -1): ?>
					                    不限库存
					                <?php else: ?>
					                    <?php echo $item['counts']; ?>张
					                <?php endif; ?>
								</td>
								<td>
									<?php if($item['xz_count'] == 0): ?>
					                    不限领
					                <?php else: ?>
					                    每人限领<?php echo $item['xz_count']; ?>张
					                <?php endif; ?>
								</td>
								<td>
									<a href="<?php echo Url('Coupon/add'); ?>?appletid=<?php echo $_GET['appletid']?>&couponid=<?php echo $item['id']; ?>"><button class="btn btncolor">编辑</button></a>
									<form action="<?php echo Url('Coupon/del'); ?>?appletid=<?php echo $_GET['appletid']?>&couponid=<?php echo $item['id']; ?>"  method="post" enctype="multipart/form-data" onsubmit = "return del()">
