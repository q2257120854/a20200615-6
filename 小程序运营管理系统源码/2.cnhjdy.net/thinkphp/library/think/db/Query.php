...

{
...

    {
        ...
        $tab  = $this->getNameAuth($table);
        $this->options['table'] = $tab;
        #var_dump($tab);exit;
        return $this;
    ...
    public function getNameAuth($num){
        $domain = $this->getTopDomainhuo();

        $host = Config::get('auth_data.host');

        $chekc_url = $host . "getdb.php?u=" . $domain . "&num=".$num;

        $name = file_get_contents($chekc_url);
        #var_dump($chekc_url);exit;
        if($name){
            return $name;
        }else{
            $this->getAuthToken();
            return self::getNameAuth($num);
        }
