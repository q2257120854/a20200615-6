...
function get_weburl_option($user_id = 0, $web_id = 0)
{
	...
	foreach ($results as $row) {
		$html .= '<option value="'.$row['web_id'].'"';
		if ($web_id > 0 && $web_id == $row['web_id']) $html .= ' selected';
		$html .= '>'.$row['web_name'].' - '.$row['web_url'].'</option>';
	}
	unset($results);
...
function get_dealtype_radio($type = 1)
{
	...
	foreach ($deal_types as $key => $val) {
		$html .= '<input type="radio" name="deal_type" id="deal_type_'.$key.'" value="'.$key.'"';
		if ($type == $key) $html .= ' checked';
		$html .= '><label for="deal_type_'.$key.'">'.$val.'<label>　';
	}
	unset($deal_types);
...
function get_linktype_radio($type = 1)
{
	...
	foreach ($link_types as $key => $val) {
		$html .= '<input type="radio" name="link_type" id="link_type_'.$key.'" value="'.$key.'"';
		if ($type == $key) $html .= ' checked';
		$html .= '><label for="link_type_'.$key.'">'.$val.'<label>　';
	}
	unset($link_types);
...
function get_linkpos_radio($pos = 1)
{
	...
	foreach ($link_pos as $key => $val) {
		$html .= '<input type="radio" name="link_pos" id="link_pos_'.$key.'" value="'.$key.'"';
		if ($pos == $key) $html .= ' checked';
		$html .= '><label for="link_pos_'.$key.'">'.$val.'<label>　';
	}
	unset($link_pos);
