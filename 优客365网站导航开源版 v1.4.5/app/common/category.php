...
function get_category_option($cate_mod = 'webdir', $root_id = 0, $cate_id = 0, $level_id = 0) 
{
	...
	foreach ($results as $value) {
		$html .= '<option value="'.$value['cate_id'].'"';
		if ($cate_id > 0 && $cate_id == $value['cate_id'])  $html .= ' selected';
		
		if ($level_id == 0) {
			$html .= ' style="background: #EEF3F7;">';
		...
		$html .= '</option>';
		$html .= get_category_option($cate_mod, $value['cate_id'],$cate_id,$level_id + 1);
	}
	unset($results);
...
function get_categories($cate_id = 0, $top_num = 0) 
{
	...
	$categories = [];
	foreach ($results as $value) {
		$value['cate_link'] = url($value['cate_mod'],['cid'=>$value['cate_id']]);
		$categories[] = $value;
	}
	unset($results);
...
function get_all_category() 
{
	...
	foreach ($results as $value) {
		$value['cate_link'] = url($value['cate_mod'],['cid'=>$value['cate_id']]);
		$categories[$value['cate_id']] = $value;
	}
	unset($results);
...
function get_one_category($cate_id = 0) 
{
	...
	$value = load_cache('category_'.$cate_id) ? load_cache('category_'.$cate_id) : $Db->query("SELECT cate_id, root_id,cate_order,cate_mod, cate_name, cate_dir, cate_arrparentid, cate_arrchildid, cate_childcount, cate_postcount FROM ".table('categories')." WHERE cate_id=$cate_id LIMIT 1",'Row');
...
function get_category_parent_ids($cate_id) 
{
	...
	foreach ($results as $value) {
		if ($value['root_id'] > 0) {
			$html .= get_category_parent_ids($value['root_id']);
			$html .= ','.$value['root_id'];
		} else {
			$html = '0';
		}
...
function get_category_child_ids($cate_id) 
{
	...
	foreach ($results as $value) {
		$html .= ','.$value['cate_id'];
		$html .= get_category_child_ids($value['cate_id']);
	}
	unset($results);
...
function get_category_model_option($model = 'website') 
{
	...
	foreach ($category_modules as $key => $val) {
		...
		if (!empty($model) && $model == $key) $html .= ' selected';
		$html .= '>'.$val.'</option>';
	}
