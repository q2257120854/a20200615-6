...
if ($action == 'list') {
	...
	foreach($query as $row){
		...
		$row['cate_operate'] = '<a href="'.$fileurl.'?mod='.$cate_mod.'&act=edit&cate_id='.$row['cate_id'].'">编辑</a>
		&nbsp;|&nbsp;<a href="'.$fileurl.'?mod='.$cate_mod.'&act=clear&cate_id='.$row['cate_id'].'" onClick="return confirm(\'注：该操作将清空此分类及其子分类下的内容！\n\n确定清空吗？\');">清空</a>&nbsp;|&nbsp;<a href="'.$fileurl.'?mod='.$cate_mod.'&act=del&cate_id='.$row['cate_id'].'" onClick="return confirm(\'注：该操作将同时删除此分类下的子分类及相关内容！\n\n确定删除吗？\');">删除</a>&nbsp;|&nbsp;
		<a href="'.$fileurl.'?mod='.$cate_mod.'&root_id='.$row['cate_id'].'">查看下级</a>&nbsp;|&nbsp;
		<a href="'.$fileurl.'?mod='.$cate_mod.'&act=add&root_id='.$row['cate_id'].'">添加下级</a>';
		$categories[] = $row;
	}
	unset($row);
