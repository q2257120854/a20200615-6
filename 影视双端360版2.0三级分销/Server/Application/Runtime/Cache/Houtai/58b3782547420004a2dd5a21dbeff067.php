...
<div class="page-content clearfix">
 <div class="search_style">
     <form action="/Houtai/User/sea" method="POST">
      <ul class="search_content clearfix">
       <li><label class="l_f">会员名称</label><input name="uname" type="text"  class="text_add" placeholder="输入会员名称、电话、邮箱"  style=" width:400px"/></li>
 ...
 <div class="sort_style">
     <div class="border clearfix">
       <span class="l_f">
        <a href="/Houtai/User/add" id="sort_add" class="btn btn-warning"><i class="fa fa-plus"></i> 添加会员</a>
		
		<a href="javascript:ovid()" onclick="shijian()"   class="btn btn-warning" style="background-color:#7EC0EE!important;border-color: #7EC0EE;"><i class="fa fa-plus"></i>批量增加时间</a>
 ...
 $time = time(); $vip = $vo['viptime']; if($vip < $time){ echo '<td class="am-hide-sm-only" style="color:red;">'.date('Y-m-d H:i:s',$vo['viptime']).'</td>'; }else{ echo '<td class="am-hide-sm-only">'.date('Y-m-d H:i:s',$vo['viptime']).'</td>'; } ?>
      ...
      <td class="td-manage">
        <a title="编辑"  href="/Houtai/User/edit?id=<?php echo ($vo["id"]); ?>"  class="btn btn-xs btn-info" ><i class="fa fa-edit bigger-120"></i></a>
